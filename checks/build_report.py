...
summary={'checked_at':max(r['checked_at'] for r in results),'total_streams':len(results),'stream_results':dict(counts),'total_channels':len(channels),'channels_with_working_stream':sum(any(r['status']=='working' for r in rs) for rs in channels.values())}
