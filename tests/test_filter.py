...
class FilterTest(unittest.TestCase):
    ...
    def test_language_category_and_duplicates_end_to_end(self):
        ...
        combined = ukr+entry('Toon.ru','toon')+entry('Film.ua','film')+entry('English.ru','english','tvg-language="eng"')+entry('Unknown.ru','unknown')+entry('News.ru','news')+entry('Film.ua','foreign','tvg-language="eng"')+entry('Film.ua','video.mp4')+entry('New.ru@RU','new')+entry('New.ru@EN','english-feed')
        def fetch(url):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(m,'ROOT',root), patch.object(m,'OUT',root/'my-iptv.m3u'), patch.object(m,'fetch',side_effect=fetch), patch.object(m,'SOURCES',[('test','https://example.com/input',None)]):
                ...
                self.assertEqual({url for _,url in result},{'https://example.com/film','https://example.com/toon','https://example.com/new','https://example.com/plusplus','https://example.com/niki'})
